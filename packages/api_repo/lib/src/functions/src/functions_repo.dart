...
class FunctionsRepoImpl implements FunctionsRepo {
  ...
  Future<ApiResult> sendSos(double lat, double lng) async {
    try {
      final result = await client.post(Endpoints.sendSos,
  ...
  // get sos notification
  @override
  Future<ApiResult<List<SosNotification>>> getSosNotification() async {
    try {
      final result = await client.get(Endpoints.getSosRecord);
      final data = result.data['data'] as List;
      final model = data.map((e) => SosNotification.fromJson(e)).toList();
